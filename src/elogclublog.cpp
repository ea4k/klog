 ...
 void eLogClubLog::slotQsoUploadFinished(QNetworkReply *data)
{
    ...
    result = data->error();
     //qDebug()<< "eLogClubLog::slotQsoUploadFinished - Result = " << QString::number(result);
    ...
    if (result == QNetworkReply::NoError)
    {
        ...
        text = "ClubLog: " + prepareToTranslate(sdata);
         //qDebug()<< sdata;
         //qDebug()<< "eLogClubLog::slotQsoUploadFinished - NO ERROR";
        if (uploadingFile)
        {
            uploadingFile = false;
    ...
    {
         //qDebug()<< "eLogClubLog::slotQsoUploadFinished - Result = Host Not found! = " << QString::number(result) ;
        text = "ClubLog: " + tr("Host not found!");
        //TODO: Mark the previous QSO as not sent to clublog
    }
    else if (result == QNetworkReply::TimeoutError)
    {
         //qDebug()<< "eLogClubLog::slotQsoUploadFinished - Result = Time out error! = " << QString::number(result) ;
    ...
    {
         //qDebug()<< "eLogClubLog::slotQsoUploadFinished - Result = Password Error! = " << QString::number(result) ;
    ...
    */
    ...
    {
         //qDebug()<< "eLogClubLog::slotQsoUploadFinished - Result = UNDEFINED = " << QString::number(result) ;
     ...
     //qDebug()<< "eLogClubLog::slotQsoUploadFinished - Result = " << QString::number(result);
     //qDebug()<< "eLogClubLog::slotQsoUploadFinished - Result Text = " << text;
    //emit done();
    //emit signalFileUploaded(result, qsos);
...
{
     //qDebug()<< "eLogClubLog::sendDataParams: Call: " << _clublogCall;
     //qDebug()<< "eLogClubLog::sendDataParams: Params: " << _params.query(QUrl::FullyEncoded).toUtf8();
     //qDebug()<< "eLogClubLog::sendDataParams: email = " << email;
     //qDebug()<< "eLogClubLog::sendDataParams: Pass = " << pass;
    ...
    if  (_clublogCall.length()>2)
    {
        params.addQueryItem("callsign",_clublogCall);
          //qDebug()<< "eLogClubLog::sendDataParams - callsign 1: " << _clublogCall;
    }
    else
    {
     ...
     //qDebug()<< "eLogClubLog::sendDataParams - query before send/delete: " << params.query(QUrl::FullyEncoded).toUtf8();
    if (_adding)
    {
    ...
    manager->post(request, postData);
     //qDebug()<< "eLogClubLog::sendDataParams - END";
    return -1;
}
...
{
      //qDebug()<< "eLogClubLog::sendLogFile: " << _file;
    qsos.clear();
    qsos.append(_qso);
    QUrl serviceUrl;
    ...
    {
          //qDebug()<< "eLogClubLog::sendLogFile: ERROR File not opened";
        return;
    }
    file->close();
    // The rest of the form goes as usual
      //qDebug()<< "eLogClubLog::sendLogFile: email: " << email;
      //qDebug()<< "eLogClubLog::sendLogFile: pass: " << pass;
      //qDebug()<< "eLogClubLog::sendLogFile: stationcall: " << stationCallsign;
      //qDebug()<< "eLogClubLog::sendLogFile: api: " << api;
