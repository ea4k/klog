...
double DataProxy_SQLite::getLowLimitBandFromBandId(const int _sm)
{
    ...
    if (sqlOK)
    {
        ...
        if (query.isValid())
        {
            ...
            {
                     //qDebug() << "DataProxy_SQLite::getLowLimitBandFromBandId: -1.0-1";
                query.finish();
                return -1.0;
            }
            else
        ...
        {
                 //qDebug() << "DataProxy_SQLite::getLowLimitBandFromBandId: -1.0-2";
            query.finish();
            return -1.0;
        }
    }
        ...
        while(query.next())
        {
            if (query.isValid())
            {
                queryString = (query.value(0)).toString();
                     //qDebug() << "DataProxy_SQLite::getBandNames: " << queryString;
...
QStringList DataProxy_SQLite::sortBandNamesBottonUp(const QStringList _qs)
{
    ...
    QStringList qs;
    qs.clear();
      //qDebug() << "DataProxy_SQLite::sortBandNamesBottonUp-00";

    for (int j=0; j<_qs.count(); j++)
    {
        map.insert(getLowLimitBandFromBandName(_qs.at(j)), _qs.at(j));
    }
      //qDebug() << "DataProxy_SQLite::sortBandNamesBottonUp-10";
    QMap<double, QString>::const_iterator i = map.constBegin();
      //qDebug() << "DataProxy_SQLite::sortBandNamesBottonUp-20";
    while (i != map.constEnd()) {
        qs << i.value();
        ++i;
    }

      //qDebug() << "DataProxy_SQLite::sortBandNamesBottonUp - END -lengh = " << QString::number(qs.length());
    qs.removeDuplicates();
    return qs;
}
...
{
         //qDebug() << "DataProxy_SQLite::getBandIDs";
    QStringList bands = QStringList();
...
{
         //qDebug() << "DataProxy_SQLite::getBandsInLog: ";
    QStringList bands = QStringList();
...
QString DataProxy_SQLite::getNameFromQRZ(const QString &_call)
{
    if (_call.length() <= 0)
    {
        //qDebug() << "DataProxy_SQLite::getNameFromQRZ return 0";
        return QString();
    }
    ...
    if (sqlOk)
    {
        while (query.next())
        {
            if (query.isValid())
            {
                ...
                {
                    //qDebug() << "DataProxy_SQLite::getNameFromQRZ: " <<  (query.value(0)).toString();
                    QString v = (query.value(0)).toString();
                    query.finish();
                    return v;
                }
        ...
        query.finish();
        //qDebug() << "DataProxy_SQLite::getNameFromQRZ: NO NAME" ;
        return QString();
    }
    else
    {
...
{
    //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: " << _call;
/*
    Mandatory data:
             "qso_date VARCHAR(10) NOT NULL, "
             "call VARCHAR(40) NOT NULL, "
...
*/
    ...
    {
        //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: Error: call";
        return -1;
    }

    if (_datetime.isValid())
    {
        //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: time-on: " <<  _datetime;
        stringFields  = stringFields  + "qso_date, ";
        QDateTime _dateTime;
        stringData =  stringData + "'" + util->getDateTimeSQLiteStringFromDateTime(_datetime) + "', ";
        //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: time-on: " << stringData;
    }
    else
    {
        //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: Error: time-on_ " << _datetime;
        return -2;
    }
    ...
    if (band != _band)
    {
        //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: Error: FREQ / BAND inconsistency " << _band << "/" << QString::number(_freq);
    }
    ...
    {
           //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: Error: band";
    ...
    {
           //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: Error: mode";
        emit queryError(Q_FUNC_INFO, "Incorrect mode: " + _mode, "-1000", "No query error");
        return -4;
    }
    ...
    stringQuery = "INSERT INTO log (" + stringFields  + ") values (" + stringData +")" ;
    //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: Query: " << stringQuery;

    bool sqlOK = query.exec(stringQuery);

    //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: LastQuery: " << query.lastQuery();
    ...
    if (sqlOK)
    {
        ...
        if (query.exec("SELECT MAX(id) FROM log"))
        {
            if (query.next())
            {
                ...
                {
                    //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: NOT VALID";
                }
            }
            else
            {
                //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: QUERY EXEC NOT NEXT";
            }
        }
        else
        {
            //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: NOT OK: " << query.lastError().databaseText();
            //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: NOT OK: " << QString::number(query.lastError().nativeErrorCode());
        }
        //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: SQL OK";
        return -5;
    }
    else
    {
        emit queryError(Q_FUNC_INFO, query.lastError().databaseText(), query.lastError().nativeErrorCode(), query.lastQuery());
        query.finish();
        //qDebug() << "DataProxy_SQLite::addQSOFromLoTW: Error: SQL ";
        return -100;
    }
}
...
QList<int> DataProxy_SQLite::getQSOsListLoTWToSend(const QString &_stationCallsign, const QString &_myGrid, const QDate &_startDate, const QDate &_endDate, bool _justQueued, int _logN)
{
    ...
    {
        _queryGrid_string = QString("(my_gridsquare!='ALL' OR my_gridsquare IS NULL OR my_gridsquare='')");
    }
    else
    {
...
QList<int> DataProxy_SQLite::getQSOsListClubLogToSent(const QString &_stationCallsign, const QDate &_startDate, const QDate &_endDate, bool _justModified, int _logN)
{
    ...
    if (doesThisLogExist (_logN))
    {
        //qDebug() << "DataProxy_SQLite::getQSOsListClubLogToSent log DOES exist";
        _query_logNumber = QString(" AND lognumber='%1'").arg(_logN);
    }
    else
    {
...
QList<int> DataProxy_SQLite::getQSOsListToBeExported(const QString &_stationCallsign, const QString &_grid, const QDate &_startDate, const QDate &_endDate, int _logN)
{
    ...
    else if (_grid == "ALL")
    {
        ...
        _queryGrid_String = QString("AND ((my_gridsquare!='ALL') OR (my_gridsquare IS NULL) OR (my_gridsquare=''))");
    }
    else
    { // NO GRID
...
bool DataProxy_SQLite::setDXCCAwardStatus(const int _qsoId)
{
    ...
    // If not worked: Add and Return true
      ...
      //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: " << QString::number(_qsoId);
    if (_qsoId <= 0)
    {
           //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: QSOid <=0 " << QString::number(_qsoId);
        return false;
    }
    QList<int> values;
    values.clear();
    ...
    if (_dxcc <= 0)
    {
           //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: DXCC <= 0: DXCC: " << QString::number(_dxcc) << " - "  << QString::number(_qsoId);
        return false;
    }
    ...
    if (_band <= 0)
    {
          //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: Band <= 0: BAND: " << QString::number(_band) << " - "  << QString::number(_qsoId);
        return false;
    }
    ...
    if (_mode <= 0)
    {
          //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: Mode <= 0: Mode: " << QString::number(_mode) << " - "  << QString::number(_qsoId);
        return false;
    }
    ...
    if (_log <= 0)
    {
          //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: Log <= 0: Log: " << QString::number(_log) << " - "  << QString::number(_qsoId);
        return false;
    }
    ...
    bool sqlOK = query.exec(queryString);
      //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: Queryexec-1: " << queryString;
    queryString.clear();
    ...
    if (sqlOK)
    {
        ...
        if (queryString.length()>5)
        {
            if (query.exec(queryString))
            {
                  //qDebug() << "DataProxy_SQLite::setDXCCAwardStatus: Queryexec-2: " << queryString;
                query.finish();
                return true;
            }
            else
...
{
    //qDebug() << "DataProxy_SQLite::fillEmptyDXCCInTheLog" ;
        ...
        while (query.next())
        {
            if (query.isValid())
            {
                ...
                _continent = getContinentShortNameFromEntity(_dxcc.toInt());
                   //qDebug() << "DataBase::fillEmptyDXCCInTheLog: DXCC: " << _dxcc;
                   //qDebug() << "DataBase::fillEmptyDXCCInTheLog: Cont: " << _continent;
                ...
                {
                       //qDebug() << "DataBase::fillEmptyDXCCInTheLog: progress canceled";
                    query.finish();
                    return true;
                }
                j++;
...
bool DataProxy_SQLite::doesThisLogExist(const int _log)
{
    ...
    bool sqlOK = query.exec(queryString);
       ...
       //qDebug() << "DataProxy_SQLite::doesThisLogExist: query: " << query.lastQuery() ;
...
QList<QSO *> DataProxy_SQLite::getGridStats(int _log)
{
    ...
    else
    {
        ...
        //qDebug() << "DataProxy_SQLite::addQSO:  Query OK";
    }
...
{//TODO: Check if I can simply remove this function
   ...
   //qDebug() << "DataProxy_SQLite::addQSO: QDateTime: " << util->getDateTimeSQLiteStringFromDateTime(qso->getDateTimeOn()) ;
    ...
    {
        //qDebug() << "DataProxy_SQLite::addQSO:  END-1";
        return -1;
    }
    ...
    {
       //qDebug() << "DataProxy_SQLite::addQSO:  END-2";
        return -2;
    }
   //qDebug() << "DataProxy_SQLite::addQSO: Band: " << qso->getBand();
    ...
    {
       //qDebug() << "DataProxy_SQLite::addQSO:  END-3";
        return -3;
    }
    ...
    {
       //qDebug() << "DataProxy_SQLite::addQSO:  END-4";
        return -4;
    }
    ...
    //qDebug() << "DataProxy_SQLite::addQSO:  getQSL_RCVD: " << qso->getQSL_RCVD();
    if (util->isValidQSL_Rcvd(qso->getQSL_RCVD()))
    {
        //qDebug() << "DataProxy_SQLite::addQSO:  getQSL_RCVD: VALID " ;
        stringFields.append("qsl_rcvd, ");
        stringData = stringData + "'" + qso->getQSL_RCVD() + "', ";
    }
    else
    {
        //qDebug() << "DataProxy_SQLite::addQSO:  getQSL_RCVD: NOT valid" ;
    }
    //qDebug() << "DataProxy_SQLite::addQSO:  LOTW_QSL_RCVD: " << qso->getLoTWQSL_RCVD() ;
    if (util->isValidQSL_Rcvd(qso->getLoTWQSL_RCVD()))
    {
        //qDebug() << "DataProxy_SQLite::addQSO:  LOTW_QSL_RCVD - VALID" ;
        stringFields.append("lotw_qsl_rcvd, ");
        stringData = stringData + "'" + qso->getLoTWQSL_RCVD() + "', ";
    }
    else
    {
        //qDebug() << "DataProxy_SQLite::addQSO:  LOTW_QSL_RCVD - NOT valid" ;
    }
    //qDebug() << "DataProxy_SQLite::addQSO:  LOTW_QSL_RCVD - END" ;
    ...
    {
       //qDebug() << "DataProxy_SQLite::addQSO: No LogNumber";
        return -2;
    }
    ...
    if (!sqlOK)
    {
       //qDebug() << "DataProxy_SQLite::addQSO:  Query NOK";
       //qDebug() << "DataProxy_SQLite::addQSO:  " << query.lastError().databaseText();
       //qDebug() << "DataProxy_SQLite::addQSO:  " << query.lastQuery();
        emit queryError(Q_FUNC_INFO, query.lastError().databaseText(), query.lastError().nativeErrorCode(), query.lastQuery());
        return -1;
    }
    else
   ...
   //qDebug() << "DataProxy_SQLite::addQSO: " << stringQuery;
    return getLastQSOid();
    //return 1;
}
