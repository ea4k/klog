...
bool DataBase::populateTableWithModes(const QStringList& submodes, const QString& mode, const QString& cabrillo, bool NoTmp)
{
    ...
    //for (const auto& indexEntityStatus : std::as_const(uniqueSubmodes))
    for (const QString& sm : std::as_const(uniqueSubmodes)) {
...
bool DataBase::populateTableMode(const bool NoTmp)
{
    ...
    for (const AdifMode &m : adif.getModeList())
        if (!populateTableWithModes(m.submodes, m.mode, m.cabrillo, NoTmp))
            ok = false;
    return ok;
}
