...
{
    //qDebug() << "FileAwardManager::importNewAwardFile"  << QT_ENDL;
    QString fileName = QFileDialog::getOpenFileName(nullptr, tr("Open Award file"), util->getHomeDir(), tr("Award files (*.awa)"));
    //qDebug() << "FileAwardManager::importNewAwardFile - file: " << fileName  << QT_ENDL;
    QFile file( fileName );
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) /* Flawfinder: ignore */
    {
        //qDebug() << "FileAwardManager::importNewAwardFile File not found" << fileName << QT_ENDL;
    ...
    {
        //qDebug() << "FileAwardManager::importNewAwardFile File opened" << QT_ENDL;
    }
    ...
    { // KLog first read the full file, to detect potential format errors and counting the number of references
        //qDebug() << "FileAwardManager::importNewAwardFile: First read, counting and looking for errors: " << line << QT_ENDL;

        line = file.readLine().trimmed().toUpper();
        //qDebug() << "FileAwardManager::importNewAwardFile: " << line << QT_ENDL;
        number = number + line.count("EOR>");
        if ((line.count("<EOH>")>0) )
        {
            hasEOH = true;
        }
        //qDebug() << " //FileAwardManager::importNewAwardFile in the while, end of loop"  << QT_ENDL;
    }
    //qDebug() << "FileAwardManager::importNewAwardFile: While finished "  << QT_ENDL;
    if (!hasEOH)
    {
    ...
    while ( !file.atEnd()   )
    {
        ...
        { // Lets read the HEADER
            ...
            foreach (aux, fields)
            {
                //qDebug() << "FileAwardManager::importNewAwardFile: Reading header: " << aux << QT_ENDL;
                aux = aux.trimmed();
                aux = "<" + aux;
                ...
                if (adifField.length()==2)
                {
                    ...
                    {
                        //qDebug() << "FileAwardManager::importNewAwardFile: Invalid AWA field found HEADER" << adifField.at(0) << QT_ENDL;
                    }
        ...
        { // NOT in EOH
            ...
            //qDebug() << "FileAwardManager::importNewAwardFile: Parsing : " << line << QT_ENDL;
            foreach (aux, fields)
            {
                aux = aux.trimmed();
                aux = "<" + aux;
                ...
                adifField.clear();
                adifField << util->getValidADIFFieldAndData(aux);
                //qDebug() << "FileAwardManager::importNewAwardFile: Length: " << QString::number(adifField.count()) << QT_ENDL;
                if (adifField.count()==2)
                {
                    //qDebug() << "FileAwardManager::importNewAwardFile: Parsing : (" << adifField.at(0) << "/" << adifField.at(1) << ")" << QT_ENDL;
                ...
                {
                    //qDebug() << "FileAwardManager::importNewAwardFile: Invalid AWA field found in BODY" << adifField.at(0) << QT_ENDL;
                }
            }}
        }
    }
...
{
    //qDebug() << "FileAwardManager::showError: " << _msg << QT_ENDL;
    QMessageBox msgBox;
