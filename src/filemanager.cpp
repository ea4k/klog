...
logfileInfo FileManager::getADIFFIleInfo(QFile & _f)
{
    ...
    while ( !file.atEnd() )
    {
        ...
        if (line.contains("ARRL Logbook of the World Status Report"))
            fileInfo.programId = "LOTW";
...
int FileManager::adifReadLog(const QString& tfileName, QString _stationCallsign, int logN, int fileIndex, int fileCount, int *importedOut, int *ignoredOut)
{
    ...
    bool lotWDownloaded = (fileInfo.programId.toUpper() == "LOTW");
    //qDebug() << Q_FUNC_INFO << " - LoTW: " << util->boolToQString(lotWDownloaded);
...
int FileManager::processQSO(QSO& qso, const QString& _stationCallsign)
{
   //qDebug() << Q_FUNC_INFO << " - Start";
    ...
    // Mode ID for validation and duplicate cache lookup: the submode id, not the parent
    // mode id. modeid is shared by every submode of the same ADIF family (e.g. FT2 and
    // FT4 are both MFSK), so using it here caused genuinely different QSOs to be flagged
    // as duplicates. The duplicate cache (loadDupeCacheBG) is keyed by submode too, so
    // this stays consistent with it. See issue #1120.
    const int modeId = dataProxy->getSubModeIdFromQSO(qso);
    if (modeId <= 0)
        return -3;
    ...
    for (QString aux : fields)
    {
        ...
        if (aux.startsWith("PROGRAMID:", Qt::CaseInsensitive))
        {
            ...
            if (idx != -1 && idx + 1 < aux.length())
                return (aux.mid(idx + 1).trimmed()).toUpper();
        }
        ...
        if (aux.contains("PROGRAMID:"))
        {
            ...
            if (programIdFields.size() > 1)
                return (programIdFields.at(1).trimmed()).toUpper();
        }
    }
    return QString();
}
